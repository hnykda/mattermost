...
func TestPropertyFieldBroadcastParams(t *testing.T) {
	...
	t.Run("empty target type returns empty strings", func(t *testing.T) {
...
// TestCreateDeletePropertyFieldPSAv1NoPanic verifies that creating and
// deleting PSAv1 fields (empty ObjectType) succeeds without panicking.
// The publishPropertyFieldEvent guard skips event publishing for PSAv1
// fields. This is a smoke test — the guard cannot be directly asserted
// at the app layer since Publish is fire-and-forget. The PSAv1 value
// opt-out is more thoroughly tested in TestUpsertPropertyValuesPSAv1OptOut
// at the API layer using a marker-event pattern.
func TestCreateDeletePropertyFieldPSAv1NoPanic(t *testing.T) {
	mainHelper.Parallel(t)
	th := Setup(t).InitBasic(t)
	...
	t.Run("create PSAv1 field with empty ObjectType should not panic", func(t *testing.T) {
		field := &model.PropertyField{
			GroupID: groupID,
			Name:    "PSAv1 Field No WS Event",
			Type:    model.PropertyFieldTypeText,
		}
		created, appErr := th.App.CreatePropertyField(th.Context, field, false, "")
		require.Nil(t, appErr)
		assert.NotEmpty(t, created.ID)
		assert.Empty(t, created.ObjectType)
	})

	t.Run("delete PSAv1 field with empty ObjectType should not panic", func(t *testing.T) {
		field := &model.PropertyField{
			GroupID: groupID,
			Name:    "PSAv1 Field Delete No WS Event",
			Type:    model.PropertyFieldTypeText,
		}
		created, appErr := th.App.CreatePropertyField(th.Context, field, false, "")
		require.Nil(t, appErr)
