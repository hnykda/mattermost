...
func TestUpsertPropertyValuesPSAv1OptOut(t *testing.T) {
	...
	t.Run("empty objectType should not publish websocket event", func(t *testing.T) {
		th.LoginBasic(t)
		webSocketClient := th.CreateConnectedWebSocketClient(t)

		// Call UpsertPropertyValues with empty objectType (PSAv1 opt-out)
		...
		// Now trigger a known event so we can detect when we've drained the channel
		...
		// Wait for the marker event to arrive; if we see a property_values_updated
		// event before it, the test fails
		require.Eventually(t, func() bool {
			select {
			case event := <-webSocketClient.EventChannel:
				// We should NOT see a property_values_updated event
...
func TestPatchPropertyValuesConnectionIDOmission(t *testing.T) {
	mainHelper.Parallel(t)
	th := SetupConfig(t, func(cfg *model.Config) {
		cfg.FeatureFlags.IntegratedBoards = true
	}).InitBasic(t)

	group, err := th.App.RegisterPropertyGroup("test_connid_omit")
	require.Nil(t, err)
	...
	t.Run("websocket event OmitConnectionId should match request Connection-Id header", func(t *testing.T) {
		th.LoginBasic(t)
		webSocketClient := th.CreateConnectedWebSocketClient(t)

		// Set a custom Connection-Id header on the client
		testConnID := model.NewId()
		th.Client.HTTPHeader[model.ConnectionId] = testConnID

		items := []model.PropertyValuePatchItem{
			{FieldID: createdField.ID, Value: json.RawMessage(`"connid-test"`)},
		}
		_, resp, patchErr := th.Client.PatchPropertyValues(context.Background(), group.Name, "post", th.BasicPost.Id, items)
		require.NoError(t, patchErr)
		CheckOKStatus(t, resp)

		// Clean up the header
		delete(th.Client.HTTPHeader, model.ConnectionId)

		require.Eventually(t, func() bool {
			select {
			case event := <-webSocketClient.EventChannel:
				if event.EventType() == model.WebsocketEventPropertyValuesUpdated {
					require.Equal(t, testConnID, event.GetBroadcast().OmitConnectionId)
					return true
				}
			default:
				return false
