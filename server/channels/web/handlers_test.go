...
func TestHandlerServeCSPHeader(t *testing.T) {
	...
	t.Run("static, without subpath", func(t *testing.T) {
		...
		handler.ServeHTTP(response, request)
		assert.Equal(t, 200, response.Code)
		assert.Equal(t, []string{"frame-ancestors 'self' " + *th.App.Config().ServiceSettings.FrameAncestors + "; script-src 'self'; object-src 'none'; base-uri 'self'; form-action 'self'"}, response.Header()["Content-Security-Policy"])
	})
	...
	t.Run("dev mode", func(t *testing.T) {
		...
		handler.ServeHTTP(response, request)
		assert.Equal(t, 200, response.Code)
		assert.Equal(t, []string{"frame-ancestors 'self' " + *th.App.Config().ServiceSettings.FrameAncestors + "; script-src 'self' 'unsafe-eval' 'unsafe-inline'; object-src 'none'; base-uri 'self'; form-action 'self'"}, response.Header()["Content-Security-Policy"])
	})
}
