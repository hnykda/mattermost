...
const (
	enCsAnalyzerName    = "en_cs"
	csLightAnalyzerName = "cs_light"
)

var keywordMapping *mapping.FieldMapping
var standardMapping *mapping.FieldMapping
var csMessageMapping *mapping.FieldMapping
...
func init() {
	...
	// Message text is indexed into two parallel fields with per-language analyzers,
	// because chaining a Czech and an English stemmer on one field makes them corrupt
	// each other's suffixes (porter's y→i rule rewrites Czech -y plurals like
	// "stromy"→"stromi" before the Czech rules see them, and the Czech vowel/e-removal
	// rules mangle English forms like "user"→"usr" or "based"→"basd"):
	//   - "Message" (en_cs): porter stemming, EN+CS stop words, diacritics folding —
	//     English matching behaves like a plain English analyzer;
	//   - "MessageCs" (cs_light): Czech light stemming (Lucene port), EN+CS stop
	//     words, diacritics folding — Czech case forms converge, with or without
	//     diacritics in either the message or the query.
	// SearchPosts queries both fields as a disjunction, so whichever analyzer fits
	// the language of the terms produces the match. Existing indexes must be purged
	// and re-indexed after upgrading to pick up the new mapping.
	standardMapping = bleve.NewTextFieldMapping()
	standardMapping.Analyzer = enCsAnalyzerName

	csMessageMapping = bleve.NewTextFieldMapping()
	csMessageMapping.Name = "MessageCs"
	csMessageMapping.Analyzer = csLightAnalyzerName
...
func addCustomAnalyzers(m *mapping.IndexMappingImpl) {
	...
	// Folding must run after the Czech stemmer: the stemmer's suffix tables need
	// diacritics intact ("ích", "ů", ...).
	err = m.AddCustomAnalyzer(csLightAnalyzerName, map[string]interface{}{
		"type":      "custom",
		"tokenizer": "unicode",
		"token_filters": []interface{}{
			"possessive_en",
	...
	if err != nil {
		panic("bleveengine: failed to register cs_light analyzer: " + err.Error())
	}
}
...
func getPostIndexMapping() *mapping.IndexMappingImpl {
	...
	postMapping.AddFieldMappingsAt("Message", standardMapping, csMessageMapping)
